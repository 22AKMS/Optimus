...
CONTROL_FILE="$RUNTIME_DIR/control.env"
CLOUD_VARS_FILE="$ROOT_DIR/cloud_variables.env"
mkdir -p "$RUNTIME_DIR"
...
ANALYTICS_FUNCTION="${ANALYTICS_FUNCTION:-}"
...
show_cloud_config() {
  load_cloud_variables
  say "Project ID: ${PROJECT_ID:-unset}"
  say "Region: ${REGION:-unset}"
  say "Cloud Run service: ${APP_SERVICE:-unset}"
  say "Sync function: ${SYNC_FUNCTION:-unset}"
  say "Analytics function: ${ANALYTICS_FUNCTION:-unset}"
}
...
edit_cloud_config() {
  ...
  while true; do
    read -r -p "Google Cloud project ID [${PROJECT_ID:-}]: " value || true
  ...
  read -r -p "Region [${REGION:-us-central1}]: " value || true
  REGION="${value:-${REGION:-us-central1}}"

  read -r -p "Cloud Run service name [${APP_SERVICE:-cve-analyzer-app}]: " value || true
  APP_SERVICE="${value:-${APP_SERVICE:-cve-analyzer-app}}"

  read -r -p "Sync function name [${SYNC_FUNCTION:-syncRecentCves}]: " value || true
  SYNC_FUNCTION="${value:-${SYNC_FUNCTION:-syncRecentCves}}"

  read -r -p "Analytics function name [${ANALYTICS_FUNCTION:-refreshTrendAnalytics}]: " value || true
  ANALYTICS_FUNCTION="${value:-${ANALYTICS_FUNCTION:-refreshTrendAnalytics}}"

  export PROJECT_ID REGION APP_SERVICE SYNC_FUNCTION ANALYTICS_FUNCTION
  save_cloud_variables
  say "Saved cloud target settings."
}
...
require_cloud_project() {
  load_cloud_variables
  [[ -n "$PROJECT_ID" ]] || { say "No cloud project is configured. Use option 16 to set it."; return 1; }
}
...
show_cloud_status() {
  ...
  gcloud run services describe "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --format='table(metadata.name,status.url,status.latestReadyRevisionName,status.conditions[0].status)' || true
}

show_cloud_url() {
  require_cloud_project || return
  gcloud run services describe "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --format='value(status.url)' || true
}

start_cloud_app() {
  require_cloud_project || return
  say "Starting Cloud Run web app $APP_SERVICE in $REGION..."
  gcloud run services update "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --scaling=auto
  say "Cloud Run web app resumed with automatic scaling."
  local url
  url="$(gcloud run services describe "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --format='value(status.url)' 2>/dev/null || true)"
  [[ -n "$url" ]] && say "URL: $url"
}
...
stop_cloud_app() {
  ...
  gcloud run services update "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --scaling=0
...
cloud_app_status() {
  require_cloud_project || return
  local scaling
  scaling="$(gcloud run services describe "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --format='value(metadata.annotations.[run.googleapis.com/scalingMode],metadata.annotations.[run.googleapis.com/manualInstanceCount])' 2>/dev/null || true)"
...
tail_cloud_logs() {
  require_cloud_project || return
  gcloud run services logs read "$APP_SERVICE" --project "$PROJECT_ID" --region "$REGION" --limit=50
}

function_url() {
  local function_name="$1"
  gcloud functions describe "$function_name" --project "$PROJECT_ID" --gen2 --region "$REGION" --format='value(serviceConfig.uri)' 2>/dev/null || true
}
...
15) Show cloud target settings
16) Edit cloud target settings
17) Exit
MENU
}
...
case "${1:-}" in
  ...
  cloud-config) show_cloud_config; exit 0 ;;
  edit-cloud-config) edit_cloud_config; exit 0 ;;
esac
...
while true; do
  ...
  case "$choice" in
    ...
    13) stop_cloud_app ;;
    14) cloud_app_status ;;
    15) show_cloud_config ;;
    16) edit_cloud_config ;;
    17) exit 0 ;;
    *) say "Invalid choice." ;;
  esac
done
