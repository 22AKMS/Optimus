...
is_name_conflict_error() {
  local output="${1:-}"
  grep -qiE "already exists|is unavailable|not available|requested entity already exists|resource.*already exists|instance .* exists|service account .* already exists|database .* already exists" <<<"$output"
}
...
for port in range(9470, 9571):
    ...
    raise SystemExit("No free proxy port found in 9470-9570")
PYPORT
}
...
ensure_firestore_database() {
  ...
  while true; do
    ...
    echo "$output" >&2
    if is_name_conflict_error "$output"; then
      retry_name_prompt FIRESTORE_DB "Firestore database ID"
      continue
    fi
    fail "Firestore database creation failed."
  done
}
...
ensure_sql_instance() {
  ...
  while true; do
    ...
    fail "Cloud SQL instance creation failed."
  done
}
...
deploy_function_with_retry() {
  ...
  while true; do
    ...
    fail "$display_label deployment failed."
  done
}
...
start_proxy() {
  ...
  if ss -ltn 2>/dev/null | grep -q ":${port}\b"; then
    fail "Cloud SQL Auth Proxy port ${port} is already in use."
  fi
...
grant_looker_reader_access_for_owner() {
  local owner_user="$1"
  local owner_password="$2"
  export PGPASSWORD="$owner_password"
  psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -p "$PROXY_PORT" -U "$owner_user" -d "$DB_NAME" <<SQL >/dev/null
...
BEGIN
  ...
  LOOP
    EXECUTE format('GRANT SELECT ON TABLE public.%I TO %I', r.tablename, '$LOOKER_DB_USER');
  END LOOP;
END \$\$;
DO \$\$
DECLARE r record;
...
grant_looker_reader_access() {
  grant_looker_reader_access_for_owner postgres "$POSTGRES_PASSWORD"
  grant_looker_reader_access_for_owner "$DB_USER" "$DB_PASSWORD"
}
